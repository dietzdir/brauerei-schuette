...
5. **Prevent Shadowing / Ghost Components & Exclusively Use Lucide Icons**:
...
### Verification & Mandatory Post-Test Cleanup Checklist

1. **Mandatory Post-Test Cleanup (Must be executed automatically after EVERY test run without waiting for reminders)**:
   - **Step 1 - Stop Background Servers**: Immediately terminate (`kill`) all local server background tasks (`npm run start`, `npm run dev`) so ports (3000) and system resources are freed.
   - **Step 2 - Delete Generated Test Artifacts**: Delete all temporary folders and test reports created by test runners (`test-results/`, `playwright-report/`).
   - **Step 3 - Delete Created Test Data from Database**: Automatically purge all test-generated database records (e.g. test orders for "Max Mustermann", "Mobile Testkunde", and temporary test user accounts in Firestore) while leaving genuine catalog and store data untouched.
